      ...
      <div className="max-w-2xl mx-auto px-5 py-6">
        ...
        {Object.keys(grouped).map(subj => (
          <div key={subj} className="mb-6">
            <h2 className="text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2 px-1">{subj}</h2>
              ...
              {grouped[subj].map(ch => (
                <Link key={ch.id} href={`/${branch.slug}/${ch.slug}`}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {ch.gate_important && (
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-950 text-amber-700 dark:text-amber-300">★ GATE</span>
                    )}
                    {ch.concepts?.length > 0 && (
                      <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400">{ch.concepts.length} concepts</span>
                    )}
                    {ch.formulas?.length > 0 && (
                      <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-purple-50 dark:bg-purple-950 text-purple-600 dark:text-purple-400">{ch.formulas.length} formulas</span>
                    )}
