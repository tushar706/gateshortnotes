...
export async function generateMetadata({ params }) {
  try {
    ...
    return {
      ...
      keywords: note.focus_keywords || [],
      openGraph: {
        title: note.meta_title || `${note.chapter_name} – Short Notes`,
        description: note.meta_description || `${note.chapter_name} short notes for GATE.`,
        images: note.og_image_url ? [{ url: note.og_image_url }] : [],
      },
    }
  } catch {
    return {}
            ...
            <div className="flex flex-col gap-2">
              {note.pyq_reference.map((p, i) => (
                <div key={i} className="flex gap-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 items-start">
                  <span className="text-xs font-bold px-2 py-1 rounded bg-amber-100 dark:bg-amber-900 text-amber-800 dark:text-amber-300 shrink-0">
                    {p.year}
                  </span>
